...
def clean_repo_skills(repo_dir: Path, dry_run: bool) -> list[str]:
    skills_dir = (repo_dir / "skills").resolve()
    repo_resolved = repo_dir.resolve()
    if skills_dir == repo_resolved or repo_resolved not in skills_dir.parents:
        raise SystemExit(f"Refusing to clean outside repo: {skills_dir}")
...
## Clean Stale Backup Skills

Use `--clean` when preparing the repository if deleted or renamed local skills should be removed from the backup:

```powershell
python scripts\\prepare_sync.py --repo-dir . --clean --dry-run
```
...
```powershell
python scripts\\check_updates.py --repo-dir . --dry-run
```
"""
...
def main() -> int:
    ...
    parser.add_argument("--clean", action="store_true", help="Remove existing repo skills before copying the current scan.")
    args = parser.parse_args()

    skills_root = Path(args.skills_root).expanduser().resolve()
    repo_dir = Path(args.repo_dir).expanduser().resolve()
    data = scan(skills_root, args.max_file_mb, args.max_skill_mb)
    cleaned = clean_repo_skills(repo_dir, args.dry_run) if args.clean else []
    ...
    if not args.dry_run:
        ...
        check_updates = Path(__file__).with_name("check_updates.py")
        if check_updates.exists():
            shutil.copy2(check_updates, scripts_dir / "check_updates.py")
        shutil.copy2(Path(__file__), scripts_dir / "prepare_sync.py")

    print(risk_summary(data))
    if args.clean:
        print(f"would_clean={len(cleaned)}" if args.dry_run else f"cleaned={len(cleaned)}")
